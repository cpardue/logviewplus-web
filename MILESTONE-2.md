...
3. **Merge** — "All" view combining every ready file into one dataset
   (`LogEntry.file` populated; File column in the grid). **DONE 2026-09-02.**
...
- **M2.3/M2.4 (2026-09-02):** "All" merged tab (shown with ≥2 files) concatenates
  ready files in insertion order, one File column in the grid, row ids keyed by
  `file+lineNo`; clicking a file tab exits merge mode (filters persist). Zip
  ingest: `.zip` accepted by drop/pick, fflate `unzipSync` on the main thread
  (synchronous, bounded to 200 members), member paths used as file names; bad
  zips surface as an error tab. Clipboard: pasted text drop (dataTransfer
  text/plain) and a "Paste" button (`navigator.clipboard.readText()`) both feed
  the normal pipeline as `pasted-YYYYMMDD-HHMMSS.log` files. New fixture
  `tests/fixtures/logs/packed.zip` generated by `scripts/gen-zip-fixture.mjs`.
